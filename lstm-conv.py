...
class Noisy_LSTM():
    def __init__(
            ...
            self.network.summary(print_fn=lambda x: self.write_to_summary(x))
            # setup callbacks
            ...
            # self.print_model_memory_usage(1)
            # self.print_model_memory_usage(2)
            # self.print_model_memory_usage(10)

    # def train(self, epochs=1, batch_size=2,validation_split=0.2):
    def train(self, epochs=1, batch_size=1,validation_split=0.2):
    # def train(self, epochs=1, batch_size=5,validation_split=0.2):
    # def train(self, epochs=50, batch_size=10,validation_split=0.2):
        ...
        filepath = os.path.join(self.folder_to_save_in, 'saved_models', f'model--{epoch}--final.hdf5')
        self.network.save(filepath)

    def print_model_memory_usage(self, batch_size):
        model = self.network
        shapes_mem_count = 0
        ...
        print(f"Model with batch size {batch_size} will use {gbytes}GB of memory")
...
if __name__ == '__main__':
    lstm = Noisy_LSTM(
             "/home/exjobb/style_transfer/numpy_dataset_256x256/rain",
            25,
            150,
            50,
            "lstm_conv_rain_with_dropout_256",
            "results_lstm_conv_rain_with_dropout_256")
